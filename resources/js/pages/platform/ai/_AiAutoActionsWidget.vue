  ...
  <div class="widget-root" :class="`density-${density}`">
    <div class="d-flex justify-space-between align-center widget-header">
      <div class="d-flex align-center gap-2 overflow-hidden">
        <VAvatar color="success" variant="tonal" :size="avatarSize" rounded>
          <VIcon icon="tabler-robot" :size="iconSize" />
        </VAvatar>
        <span class="widget-title text-medium-emphasis">{{ t('platformAi.widgets.autoActions') }}</span>
      </div>
    </div>
    ...
    <div class="widget-body">
      ...
      <template v-else>
        <div class="widget-kpi font-weight-bold text-success">{{ data.data?.total ?? 0 }}</div>
        <div class="widget-subtext text-medium-emphasis">{{ t('platformAi.widgets.autoActionsDetail', { fills: data.data?.auto_fills ?? 0, rejects: data.data?.auto_rejects ?? 0 }) }}</div>
      </template>
    </div>
  </div>
</template>
