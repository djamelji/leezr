...
<template>
  <div class="widget-root" :class="`density-${density}`">
    <div class="d-flex justify-space-between align-center widget-header">
      <div class="d-flex align-center gap-2 overflow-hidden">
        <VAvatar color="info" variant="tonal" :size="avatarSize" rounded>
          <VIcon icon="tabler-gauge" :size="iconSize" />
        </VAvatar>
        <span class="widget-title text-medium-emphasis">{{ t('platformAi.widgets.confidence') }}</span>
      </div>
    </div>
    ...
    <div class="widget-body">
      ...
      <template v-else>
        <div class="widget-kpi font-weight-bold text-info">{{ data.data?.extraction_rate ?? 0 }}%</div>
        <div class="widget-subtext text-medium-emphasis">{{ t('platformAi.widgets.confidenceDetail', { lowCount: data.data?.low_confidence ?? 0 }) }}</div>
      </template>
    </div>
  </div>
</template>
