...
const total = computed(() => (data.value?.data?.expirations ?? 0) + (data.value?.data?.mismatches ?? 0))
</script>
  ...
  <div class="widget-root" :class="`density-${density}`">
    <div class="d-flex justify-space-between align-center widget-header">
      <div class="d-flex align-center gap-2 overflow-hidden">
        <VAvatar color="warning" variant="tonal" :size="avatarSize" rounded>
          <VIcon icon="tabler-alert-triangle" :size="iconSize" />
        </VAvatar>
        <span class="widget-title text-medium-emphasis">{{ t('platformAi.widgets.detections') }}</span>
      </div>
    </div>
    ...
    <div class="widget-body">
      ...
      <template v-else>
        <div class="widget-kpi font-weight-bold text-warning">{{ (data.data?.expirations ?? 0) + (data.data?.mismatches ?? 0) }}</div>
        <div class="widget-subtext text-medium-emphasis">{{ t('platformAi.widgets.detectionsDetail', { expirations: data.data?.expirations ?? 0, mismatches: data.data?.mismatches ?? 0 }) }}</div>
      </template>
    </div>
  </div>
</template>
