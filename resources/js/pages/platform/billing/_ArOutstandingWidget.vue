...
const amountColor = computed(() => {
  ...
  return props.data.outstanding > 0 ? 'warning' : 'success'
})

const formattedAmount = computed(() => {
  if (!props.data) return '—'
  if (props.data.currency === 'MULTI') return props.data.outstanding.toFixed(2)

  return formatMoney(Math.round(props.data.outstanding * 100), { currency: props.data.currency || 'EUR' })
})
</script>
    ...
    <div class="d-flex justify-space-between align-center widget-header">
      <div class="d-flex align-center gap-2 overflow-hidden">
        ...
        <span class="widget-title text-medium-emphasis">
          {{ t('platformBilling.widgets.arOutstanding') }}
        </span>
      </div>
    ...
    <div class="widget-body">
      ...
      <template v-else>
        ...
        <div class="widget-subtext text-medium-emphasis">
          {{ t('platformBilling.widgets.arOutstandingDesc') }}
        </div>
      </template>
    </div>
  </div>
