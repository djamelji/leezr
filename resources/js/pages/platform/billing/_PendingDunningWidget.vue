...
const kpiColor = computed(() => {
  if (!props.data) return 'warning'

  return props.data.count > 0 ? 'warning' : 'success'
})
</script>
    ...
    <div class="d-flex justify-space-between align-center widget-header">
      <div class="d-flex align-center gap-2 overflow-hidden">
        ...
        <span class="widget-title text-medium-emphasis">
          {{ t('platformBilling.widgets.pendingDunning') }}
        </span>
      </div>
    </div>
    ...
    <div class="widget-body">
      ...
      <template v-else>
        ...
        <div class="widget-subtext text-medium-emphasis">
          {{ t('platformBilling.widgets.pendingDunningDesc') }}
        </div>
      </template>
    </div>
  </div>
