...
// Sub-component — no definePage — hub: platform/communications/[tab].vue
...
const headers = computed(() => [
  ...
  { title: t('support.sla'), key: 'sla_status', width: 140, sortable: false },
...
const getOverallSla = item => {
  const sla = item.sla_status
  if (!sla) return 'on_track'

  // Worst status wins: breached > warning > on_track
    ...
    >
      ...
      >
        <VCard>
          <VCardText class="text-center">
            <div class="text-h4">
              {{ store.metrics.total_open ?? (store.metrics.open + store.metrics.in_progress + store.metrics.waiting_customer) }}
            </div>
            <div class="text-body-2 text-medium-emphasis">
              {{ t('support.slaOpenTickets') }}
            </div>
          </VCardText>
        </VCard>
      </VCol>
      ...
      >
        <VCard>
          <VCardText class="text-center">
            <div class="text-h4">
              {{ formatHours(store.metrics.avg_response_time) }}
            </div>
            <div class="text-body-2 text-medium-emphasis">
              {{ t('support.slaAvgResponseTime') }}
            </div>
          </VCardText>
        </VCard>
      </VCol>
      ...
      >
        <VCard>
          <VCardText class="text-center">
            <div
              class="text-h4"
              :class="(store.metrics.sla_compliance_percent ?? 100) < 80 ? 'text-error' : 'text-success'"
            >
              {{ store.metrics.sla_compliance_percent ?? 100 }}%
            </div>
            <div class="text-body-2 text-medium-emphasis">
              {{ t('support.slaCompliance') }}
            </div>
          </VCardText>
        </VCard>
      </VCol>
      ...
      >
        <VCard>
          <VCardText class="text-center">
            <div
              class="text-h4"
              :class="(store.metrics.total_breached ?? 0) > 0 ? 'text-error' : ''"
            >
              {{ store.metrics.total_breached ?? 0 }}
            </div>
            <div class="text-body-2 text-medium-emphasis">
              {{ t('support.slaBreachedTickets') }}
            </div>
          </VCardText>
        </VCard>
      </VCol>
      ...
      <VCardText>
        ...
        >
          ...
          <template #item.sla_status="{ item }">
            <VChip
              :color="slaColors[getOverallSla(item)]"
              size="small"
              variant="tonal"
            >
              {{ slaLabels[getOverallSla(item)]() }}
            </VChip>
          </template>
