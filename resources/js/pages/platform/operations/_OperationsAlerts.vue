...
import AlertTimeline from './_AlertTimeline.vue'
...
// ── Detail dialog (ADR-469) ─────────────────────────
const detailDialogVisible = ref(false)
const detailAlert = ref(null)
const detailLoading = ref(false)

const openDetailDialog = async alert => {
  detailLoading.value = true
  detailDialogVisible.value = true
  try {
    detailAlert.value = await store.fetchAlertDetail(alert.id)
  }
  catch {
    ...
    >
      <VCard>
        ...
        <VCardText>
          ...
          <!-- Alert info + timeline -->
          <template v-else-if="detailAlert">
            <div class="mb-4">
              <div class="d-flex align-center gap-2 mb-2">
                <VChip
                  :color="severityColors[detailAlert.severity]"
                  size="small"
                  :prepend-icon="severityIcons[detailAlert.severity]"
                >
                  {{ t(`alerts.${detailAlert.severity}`) }}
                </VChip>
                <VChip
                  :color="sourceColors[detailAlert.source]"
                  size="small"
                  variant="tonal"
                  :prepend-icon="sourceIcons[detailAlert.source]"
                >
                  {{ t(`alerts.${detailAlert.source}`) }}
                </VChip>
                <VChip
                  :color="statusColors[detailAlert.status]"
                  size="small"
                  variant="tonal"
                >
                  {{ t(`alerts.${detailAlert.status}`) }}
                </VChip>
              </div>
            ...
            <AlertTimeline :timeline="detailAlert.incident_timeline || []" />
          </template>
        </VCardText>
      </VCard>
    </VDialog>
