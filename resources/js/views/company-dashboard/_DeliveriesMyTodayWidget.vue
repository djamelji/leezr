  ...
  <div class="widget-root" :class="`density-${density}`">
    <div class="d-flex justify-space-between align-center widget-header">
      <div class="d-flex align-center gap-2 overflow-hidden">
        <VAvatar color="primary" variant="tonal" :size="avatarSize" rounded>
          <VIcon icon="tabler-package" :size="iconSize" />
        </VAvatar>
        <span class="widget-title text-medium-emphasis">{{ t('shipments.widgets.myToday') }}</span>
      </div>
    </div>
    ...
    <div class="widget-body">
      ...
      <template v-else>
        ...
        <div class="widget-subtext text-medium-emphasis">{{ t('shipments.widgets.myTodayDesc') }}</div>
      </template>
    </div>
  </div>
</template>
