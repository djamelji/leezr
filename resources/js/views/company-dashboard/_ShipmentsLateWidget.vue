  ...
  <div class="widget-root" :class="`density-${density}`">
    <div class="d-flex justify-space-between align-center widget-header">
      <div class="d-flex align-center gap-2 overflow-hidden">
        <VAvatar color="error" variant="tonal" :size="avatarSize" rounded>
          <VIcon icon="tabler-alert-triangle" :size="iconSize" />
        </VAvatar>
        <span class="widget-title text-medium-emphasis">{{ t('shipments.widgets.late') }}</span>
      </div>
    </div>
    ...
    <div class="widget-body">
      ...
      <template v-else>
        <div class="widget-kpi font-weight-bold text-error">{{ data.data?.count ?? 0 }}</div>
        <div class="widget-subtext text-medium-emphasis">{{ t('shipments.widgets.lateDesc') }}</div>
      </template>
    </div>
  </div>
</template>
